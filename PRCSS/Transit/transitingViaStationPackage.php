...
if($to == $vs || $vs == null) {
    ...
    if ($lb > 0) {
        $sqlUpdatepending = mysqli_query($con,"UPDATE pending_packages SET no_of_packages='$lb' WHERE pwblt_No='$pwblt' AND transittype='VSS' AND code='$prevcode'");
    } else {
        $sqlDeletepending = mysqli_query($con, "DELETE FROM pending_packages WHERE pwblt_No='$pwblt' AND transittype='VSS' AND code='$prevcode'");
    }
} else {
